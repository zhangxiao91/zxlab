...
export class AnnotationResponder {
  ...
  async respond(input: AnnotationInput): Promise<AnnotationResponse> {
    ...
    const replyDraft = await this.llm.replyToAnnotation({ item, selectedText: input.selectedText, comment: input.comment, action: input.action, memories: relevantMemories });
