...
export const MARKET_AGENT_THESIS_IMPACT_BUDGET_MS = 12_000;
...
export function marketAgentRunLeaseMs(
  mode: "disabled" | "shadow" | "enabled",
  dossierMode: "disabled" | "fact_only" | "enabled" = "disabled",
): number {
  return mode === "disabled" && dossierMode === "disabled" ? MARKET_AGENT_LEGACY_RUN_LEASE_MS : MARKET_AGENT_RUN_LEASE_MS;
}

export function marketAgentRunStreamTimeoutMs(
  mode: "disabled" | "shadow" | "enabled",
  dossierMode: "disabled" | "fact_only" | "enabled" = "disabled",
): number {
  return marketAgentRunLeaseMs(mode, dossierMode) + MARKET_AGENT_RUN_STREAM_GRACE_MS;
}
