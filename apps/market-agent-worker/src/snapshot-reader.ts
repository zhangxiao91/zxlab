...
export class MarketSnapshotAdapter implements CurrentMarketSnapshotReader {
  private readonly options: { service?: Fetcher; baseUrl?: string; token?: string };
  constructor(options: { service?: Fetcher; baseUrl?: string; token?: string }) { this.options = options; }
  async getCurrentSnapshot(input: Parameters<CurrentMarketSnapshotReader["getCurrentSnapshot"]>[0]) {
    ...
    const response = this.options.service ? await this.options.service.fetch(new Request(url, { headers })) : await fetch(url, { headers });
