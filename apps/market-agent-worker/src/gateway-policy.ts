...
export const THESIS_IMPACT_GATEWAY_TASK = "market-agent-thesis-impact" as const;
...
export type MarketAgentGatewayTask = typeof MARKET_AGENT_CLOSE_REVIEW_GATEWAY_TASK | typeof MARKET_AGENT_ANSWER_GATEWAY_TASK | typeof FINANCIAL_TOOL_PLANNER_GATEWAY_TASK | typeof THESIS_IMPACT_GATEWAY_TASK;
...
export function isAllowedGatewayCall(input: { caller: string; task: string }): boolean {
  return input.caller === MARKET_AGENT_ALLOWED_CALLER && (
    ...
    || input.task === THESIS_IMPACT_GATEWAY_TASK
  );
}
