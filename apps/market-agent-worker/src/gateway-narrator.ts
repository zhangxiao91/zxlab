...
import { resolveNarrativeDepthPolicy } from "./narrative-depth.ts";
...
export class GatewayNarrator implements Narrator {
  ...
  private async request(input: NarrationInput, repairIssues?: string[]): Promise<unknown> {
    ...
    const narrativeDepthPolicy = resolveNarrativeDepthPolicy({ workflow: input.workflow, askScope: input.askScope, context: evidenceContext, evidenceAssessment: input.evidenceAssessment });
