...
import type { UploadPhase } from "./lib/device-api";
...
export function SendPanel({ credential, onSent }: { credential: DeviceCredential; onSent: (item: DropItem) => void }) {
  ...
  const [phase, setPhase] = useState<"preparing" | UploadPhase>("preparing");
  ...
  async function submit() {
    ...
    setPhase("preparing");
    try {
      const created = await sendDrop(credential, targetId, payload, idempotencyKey.current);
      let delivered = created;
      if (file) {
        const upload = uploadDropFile(credential, created, file, setProgress, setPhase);
        abortUpload.current = upload.abort;
        delivered = await upload.promise;
      } else {
        setPhase("finalizing");
      }
      onSent(delivered);
  ...
  return <section className="send-panel">
    ...
    <textarea value={text} onChange={(event) => { setText(event.target.value); setFile(null); idempotencyKey.current = crypto.randomUUID(); }} placeholder="粘贴文字或链接…" maxLength={20_000} />
    ...
    <button className="utility-primary send-submit" onClick={() => void submit()} disabled={!targetId || status === "sending"}>
      {status === "sending" ? <><LoaderCircle className="spin" size={18} /> {phase === "finalizing" ? "正在完成投递" : progress ? `正在上传 ${progress}%` : "正在准备"}</> : status === "success" ? <><Check size={18} /> 投递完成</> : <><Send size={18} /> 发送到设备</>}
    </button>
    {status === "sending" && abortUpload.current && <button className="inbox-action secondary" onClick={() => abortUpload.current?.()}>取消上传</button>}
