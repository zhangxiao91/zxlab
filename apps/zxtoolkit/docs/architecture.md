...
- 步数：Android 在用户授权 `READ_STEPS` 后直接调用 Health Connect 的 aggregate API，以本地时区聚合当天 `StepsRecord.COUNT_TOTAL`；设备支持时额外请求 `READ_HEALTH_DATA_IN_BACKGROUND`，供 WorkManager 更新 Pulse。vivo 未向 Health Connect 提供数据且标准聚合为空时，客户端读取系统公开的 `vivo_settings_realtime_steps` 当日值作为厂商兼容层。启用 Pulse 后，今日精确步数会随精确电量和充电状态进入加密快照；不会读取或同步其他 Health Connect 数据。
