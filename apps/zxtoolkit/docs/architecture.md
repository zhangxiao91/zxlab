...
- 步数：Android 在用户授权 `READ_STEPS` 后直接调用 Health Connect 的 aggregate API，以本地时区聚合当天 `StepsRecord.COUNT_TOTAL`。启用 Pulse 后，今日精确步数会随精确电量和充电状态进入加密快照；不会读取或同步其他 Health Connect 数据。
