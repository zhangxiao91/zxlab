...
export async function getLLMUsageDashboard(db: LLMUsageDatabase, range: LLMUsageRange) {
  ...
  const models = await db.prepare(`SELECT provider, provider_instance, model, capability_tier, COUNT(*) attempts, SUM(CASE WHEN status='success' THEN 1 ELSE 0 END) successes, SUM(total_tokens) total_tokens, SUM(estimated_cost_usd) estimated_cost_usd, AVG(latency_ms) average_latency_ms FROM llm_usage_events WHERE created_at >= ? GROUP BY provider, provider_instance, model, capability_tier ORDER BY attempts DESC LIMIT 20`).bind(start).all<Row>();
  ...
  const errors = await db.prepare(`SELECT created_at, source, provider, provider_instance, model, candidate_id, capability_tier, provider_status_code, error_type, error_code, request_id FROM llm_usage_events WHERE created_at >= ? AND status != 'success' ORDER BY created_at DESC LIMIT 20`).bind(start).all<Row>();
  const routing = await db.prepare(`SELECT selected_tier, selection_source, reason_code, selector_provider, selector_model,
    selector_fallback_used, COUNT(*) requests, AVG(selector_attempts) average_selector_attempts
    FROM llm_routing_events WHERE created_at >= ?
    GROUP BY selected_tier, selection_source, reason_code, selector_provider, selector_model, selector_fallback_used
    ORDER BY requests DESC LIMIT 30`).bind(start).all<Row>();
  return {
    range, updatedAt: new Date().toISOString(), summary: { logicalRequests: n(totals.logical_requests), providerAttempts: n(totals.attempts), successfulRequests: successful, failedRequests: Math.max(0, n(totals.logical_requests) - successful), successRate: n(totals.logical_requests) ? successful / n(totals.logical_requests) : null, inputTokens: n(totals.input_tokens), outputTokens: n(totals.output_tokens), cachedInputTokens: n(totals.cached_input_tokens), reasoningTokens: n(totals.reasoning_tokens), totalTokens: n(totals.total_tokens), estimatedCostUsd: totals.estimated_cost_usd === null || totals.estimated_cost_usd === undefined ? null : n(totals.estimated_cost_usd), averageLatencyMs: totals.average_latency_ms === null || totals.average_latency_ms === undefined ? null : n(totals.average_latency_ms), fallbackRequests, fallbackRecoveredRequests: requests.results.filter((row) => n(row.depth) > 0 && n(row.succeeded) === 1).length }, models: models.results, sources: sources.results, trend: trend.results, fallbacks: fallbacks.results, routing: routing.results, recentErrors: errors.results,
  };
}
