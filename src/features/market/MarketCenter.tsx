...
import {
  marketStatusPresentation,
  quoteQualityPresentation,
} from "./presentation";
import type {
  MarketBar,
  MarketDataQuality,
...
const emptyState = (): MarketCenterState => ({
  quotes: [],
  bars: {},
  news: [],
  announcements: [],
...
export default function MarketCenter() {
  ...
  const save = (items: MarketWatchlistItem[]) => {
    setWatchlist(items);
    if (!items.some((item) => item.instrumentId === selectedId))
      setSelectedId(items[0]?.instrumentId ?? "");
  ...
  const addHoldings = () => {
    ...
    save([...watchlist, ...holdingsWatchlist]);
    setError(null);
  };

  const healthLabel =
    state.quality.status === "operational"
      ? "Market API"
    ...
    <div className="risk-app market-app">
      <header className="risk-appbar">
        <a href="/lab" className="risk-brand">
          ...
          <span>
            <strong>Market Center</strong>
            <small>Quotes, news, providers</small>
          </span>
        </a>
      ...
      <main className="risk-main market-main">
        <header className="market-hero">
          <div>
            <p>行情、K 线、公告与消息面统一入口</p>
            <h1>Market Center 承接估值事实，Risk 只消费它。</h1>
            <span>
              价格质量、上游 fallback、7x24
              新闻和上市公司公告会在这里先被看见，再进入风险台。
            </span>
          </div>
          <aside className="market-hero__panel">
            <p>行情中心</p>
            <strong>{state.quotes.length} 个标的</strong>
            <small>
              {state.news.length} 条消息面 · {state.announcements.length} 条公告
            </small>
            <span>
              {state.providers?.strategy ?? "sequential-fallback"} ·{" "}
              {autoRefresh
                ? `报价与当前 K 线 ${pollIntervalMs / 1000} 秒轮询`
                : "自动刷新已暂停"}
            </span>
            <a className="market-hero__link" href="/lab/risk">
              回到 Risk 工作台
            </a>
          </aside>
        </header>
        ...
        <section className="market-grid">
          <article className="market-panel market-panel--wide">
            <header>
              <div>
                <span>自选报价</span>
                <h2>{watchlist.length} 个标的</h2>
              </div>
              <div className="market-add">
                <button className="market-add__holdings" onClick={addHoldings}>
                  从当前持仓生成自选
                </button>
                <input
                  value={draft}
                ...
                />
                <button onClick={addInstrument}>添加标的</button>
              </div>
            </header>
            <div className="position-table-wrap">
              <table className="position-table market-table">
                <thead>
                  <tr>
                    <th>标的</th>
                    <th>价格</th>
                ...
                <tbody>
                  {watchlist.map((item) => {
                    const quote = state.quotes.find(
                      (entry) => entry.instrumentId === item.instrumentId,
                    );
                    const presentation = quoteQualityPresentation(quote);
                    return (
                      <tr
                        key={item.instrumentId}
                        className={
                          selectedId === item.instrumentId ? "is-selected" : ""
                        }
                      >
                        <td>
                          <button
                            className="market-instrument-button"
                            onClick={() => setSelectedId(item.instrumentId)}
                          >
                            <strong>{item.label}</strong>
                            <span>
                              {item.instrumentId} · {item.reason}
                            </span>
                          </button>
                        </td>
                        <td>
                          <strong>
                            {quote?.price == null
                              ? "—"
                              : number.format(quote.price)}
                          </strong>
                        </td>
                        <td>
                          <span
                            className={`quality-tag quality-tag--${presentation.tone}`}
                          >
                            {presentation.label}
                          </span>
                        </td>
                        <td>
                          {quote?.turnover == null
                            ? "—"
                            : compact.format(quote.turnover)}
                        </td>
                        <td>{quote?.source ?? "—"}</td>
                        <td>{time(quote?.marketTimestamp)}</td>
                        <td>
                          <button
                            ...
                            onClick={() =>
                              save(
                                watchlist.filter(
                                  (entry) =>
                                    entry.instrumentId !== item.instrumentId,
                                ),
                              )
                            }
                          >
                  ...
                  {!watchlist.length && (
                    <tr className="market-empty-row">
                      <td colSpan={7}>
                        尚未选择行情标的。可从当前持仓生成，或输入证券代码添加。
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <article className="market-panel">
            <header>
              <div>
                <span>价格质量</span>
                <h2>{selectedId || "尚未选择标的"}</h2>
              </div>
              <div className="market-chart-controls">
                <button
                  className={interval === "1m" ? "is-active" : ""}
                  onClick={() => setIntervalType("1m")}
                >
                  1分钟
                </button>
                <button
                  className={interval === "1d" ? "is-active" : ""}
            ...
            <CandlestickPanel bars={selectedBars} interval={interval} />
            <dl className="market-kv">
              <div>
                <dt>现价</dt>
                <dd>
                  {selectedQuote?.price == null
                    ? "—"
                    : number.format(selectedQuote.price)}
                </dd>
              </div>
              <div>
                <dt>昨收</dt>
                <dd>
                  {selectedQuote?.previousClose == null
                    ? "—"
                    : number.format(selectedQuote.previousClose)}
                </dd>
              </div>
              <div>
                <dt>来源</dt>
                <dd>{selectedQuote?.source ?? "—"}</dd>
              </div>
              <div>
                <dt>{interval === "1m" ? "1分钟价格点数" : "日K点数"}</dt>
                <dd>{selectedBars.length}</dd>
              </div>
            </dl>
            {selectedQuote?.warnings.map((warning) => (
              <p key={warning} className="duplicate-note">
                {warning}
              </p>
            ))}
          </article>
          <article className="market-panel">
            ...
            {state.status.map((item) => (
              <div className="setting-row" key={item.exchange}>
                <span>
                  <i className={`source-dot source-dot--${statusDot(item)}`} />
                  {item.exchange}
                </span>
                <strong>
                  {marketStatusPresentation(item)} · {item.source}
                </strong>
              </div>
            ))}
            <div className="setting-row">
              <span>Provider 策略</span>
              <strong>{state.providers?.strategy ?? "—"}</strong>
            </div>
            <div className="setting-row">
              <span>自动刷新</span>
              <strong>
                {autoRefresh ? `${pollIntervalMs / 1000} 秒` : "暂停"}
              </strong>
            </div>
            <div className="setting-row">
              <span>超时</span>
              <strong>{state.providers?.timeoutMsPerProvider ?? "—"} ms</strong>
            </div>
          </article>
          <article className="market-panel market-panel--half">
            <header>
              <div>
                <span>消息面</span>
                <h2>7x24 + 个股新闻</h2>
          ...
          <article className="market-panel market-panel--half">
            ...
            <NewsList items={state.announcements} />
          </article>
          <article className="market-panel market-panel--wide">
            <header>
              <div>
                <span>上游健康</span>
                <h2>
                  {state.attempts.length} 次 provider attempt ·{" "}
                  {state.quality.unavailableCapabilities.length} 项不可用
                </h2>
              </div>
            </header>
            <div className="market-attempts">
              {state.attempts.slice(0, 18).map((attempt, index) => (
                <div
                  key={`${attempt.provider}-${index}`}
                  className={attempt.ok ? "is-ok" : "is-bad"}
                >
                  <strong>{attempt.provider}</strong>
                  <span>
                    {attempt.ok ? "ok" : (attempt.errorCode ?? "failed")} ·{" "}
                    {attempt.latencyMs}ms
                  </span>
                </div>
              ))}
            </div>
            {state.quality.unavailableCapabilities.map((id) => (
              <p key={id} className="data-warning">
                不可用 capability：{id}
              </p>
            ))}
    ...
    <div className="market-news-list">
      {items.slice(0, 12).map((item) => (
        <a key={item.id} href={item.url} target="_blank" rel="noreferrer">
          ...
          <span>
            {item.instrumentId
              ? `${item.instrumentId} · ${item.source}`
              : item.source}
          </span>
          {item.summary && <p>{item.summary}</p>}
        </a>
      ))}
      {!items.length && <p className="empty-note">暂无可显示条目。</p>}
    </div>
  );
}
...
function CandlestickPanel({
  ...
  const closeBars = useMemo(
    () => bars.filter((item) => item.close != null),
    [bars],
  );
  useEffect(() => {
    ...
    void import("../risk/chart").then(
      ({ createCandlestickChart, createMarketLineChart }) => {
  ...
  if (chartKind === "empty")
    return <div className="market-candlestick is-empty">暂无可用价格序列</div>;
  return (
    <div
      ref={ref}
      className="market-candlestick"
      role="img"
      aria-label={
        chartKind === "candlestick"
          ? `${interval === "1m" ? "1分钟" : "日"}K 线蜡烛图`
          : `${interval === "1m" ? "1分钟" : "日"}收盘价折线图`
      }
    />
  );
}
