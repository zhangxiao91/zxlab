...
import type { MarketBar, MarketNewsItem, MarketProviderAttempt, MarketProviders, MarketQuote, MarketStatus, MarketWatchlistItem } from "./types";
...
export default function MarketCenter() {
  ...
  const refresh = async () => {
    ...
    try {
      const [quotes, providers, sse, szse, news, ...barResults] = await Promise.allSettled([
        client.getQuotes(ids),
        client.getProviders(),
        client.getStatus("SSE"),
        client.getStatus("SZSE"),
        client.getNews(ids, 36),
        ...ids.map((id) => client.getBars(id, "1d")),
      ]);
      ...
      } else next.warnings.push(errorText("news", news.reason));
      barResults.forEach((result, index) => {
        const id = ids[index];
        if (result.status === "fulfilled") next.bars[id] = result.value.data;
        else next.warnings.push(errorText(`bars:${id}`, result.reason));
      });
      setState(next);
    } catch (reason) {
  ...
  useEffect(() => { void refresh(); }, [ids.join(",")]);
...
function Sparkline({ bars }: { bars: MarketBar[] }) {
  const points = bars.slice(-44).filter((item) => item.close != null);
  if (points.length < 2) return <div className="market-sparkline is-empty">等待 K 线</div>;
  const values = points.map((item) => item.close as number);
  const min = Math.min(...values); const max = Math.max(...values); const span = Math.max(max - min, 1e-8);
  const path = values.map((item, index) => `${index === 0 ? "M" : "L"} ${(index / Math.max(values.length - 1, 1)) * 100} ${64 - ((item - min) / span) * 56}`).join(" ");
  return <svg className="market-sparkline" viewBox="0 0 100 70" preserveAspectRatio="none" role="img" aria-label="收盘价趋势"><path d={path}/></svg>;
}
