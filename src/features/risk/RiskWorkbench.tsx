...
const confidence = (input: number) => input >= 0.85 ? "高" : input >= 0.65 ? "中" : "低";
const marketUnavailable = (data: RiskDashboardData) => Boolean(data.diagnostics.market.errors.length) || data.sourceHealth.some((source) => source.name === "ApiMarketDataProvider" && source.status === "offline");
const marketModeLabel = (data: RiskDashboardData) => marketUnavailable(data) ? "API 行情不可用" : "真实 API 行情";
const marketFailureDetail = (data: RiskDashboardData) => data.diagnostics.market.errors[0] ?? data.sourceHealth.find((source) => source.name === "ApiMarketDataProvider")?.freshness ?? "全部行情上游不可用";
const marketStatusDetail = (data: RiskDashboardData) => marketUnavailable(data) ? marketFailureDetail(data) : data.sourceHealth.find((source) => source.name === "ApiMarketDataProvider")?.freshness ?? (data.reconciliation.unresolved ? "对账待处理" : "已对账");
const reliableDetail = (data: RiskDashboardData) => data.diagnostics.market.snapshotStatus === "closed-snapshot" ? "交易与对账通过质量检查，行情按闭市快照用于复盘。" : "交易、对账与报价通过质量检查。";
const quoteQualityLabel = (position: Position, data: RiskDashboardData) => data.diagnostics.market.snapshotStatus === "closed-snapshot" && position.quoteQuality === "stale" ? "闭市快照" : position.quoteQuality === "live" ? "实时" : position.quoteQuality === "cached" ? "缓存" : position.quoteQuality === "unavailable" ? "不可用" : "盘中过期";
const quoteQualityClass = (position: Position, data: RiskDashboardData) => data.diagnostics.market.snapshotStatus === "closed-snapshot" && position.quoteQuality === "stale" ? "live" : position.quoteQuality;
...
function ReliabilityBand({ data }: { data: RiskDashboardData }) {
  return <div className={`reliability-band ${data.portfolio.reliable ? "is-reliable" : "is-blocked"}`}><span className="reliability-band__signal" aria-hidden="true"/><div><strong>{data.portfolio.reliable ? "风险快照可可靠计算" : "风险结果已降低可信度"}</strong><p>{data.portfolio.reliable ? reliableDetail(data) : data.dataWarnings.slice(0, 3).join("；") || "存在未解决的数据质量问题。"}</p></div><span className="reliability-band__time">更新 {new Date(data.receivedAt).toLocaleTimeString("zh-CN")}</span></div>;
}
...
function PositionFactGrid({ data }: { data: RiskDashboardData }) {
  const brokerCount = data.brokerSnapshot?.positions.length ?? data.reconciliation.items.filter((item) => item.brokerQuantity != null).length;
  return <section className="position-fact-grid"><article><span>账本推导</span><strong>{data.positions.length} 个持仓</strong><p>{data.transactions.length} 条交易事件按时间顺序重建，不被券商快照覆盖。</p></article><article><span>券商快照</span><strong>{brokerCount ? `${brokerCount} 个标的` : "待确认"}</strong><p>{data.brokerSnapshot ? `${data.brokerSnapshot.accountName ?? "未命名账户"} · ${new Date(data.brokerSnapshot.snapshotAt).toLocaleString("zh-CN")}` : "可粘贴券商持仓文本或 CSV 生成草稿。"}</p></article><article><span>行情估值</span><strong>{marketModeLabel(data)}</strong><p>{marketStatusDetail(data)}</p></article></section>;
}

function Positions({ data, onSaveBroker, onOpenHoldingsPaste }: { data: RiskDashboardData; onSaveBroker: (id: string, quantity: number, cost: number | null) => Promise<void>; onOpenHoldingsPaste: () => void }) { return <div className="risk-view"><header className="view-intro review-intro"><div><p>交易重建持仓</p><h2>账本是事实，券商快照用于对账或建立期初持仓。</h2><span>买入、卖出、费用、税费与 adjustment 均按时间顺序处理；采用券商持仓时会明确替换现有账本。</span></div><button onClick={onOpenHoldingsPaste}>粘贴券商持仓</button></header><PositionFactGrid data={data}/><PositionTable data={data}/><Reconciliation data={data} onSave={onSaveBroker}/><div className="position-detail-grid">{data.positions.map((position) => <article key={position.instrumentId}><div><span>{position.symbol}</span><span>{position.industry}</span></div><h3>{position.name}</h3><p>{position.themes.join(" / ")}</p><dl><div><dt>平均成本</dt><dd>{value(position.averageCost)}</dd></div><div><dt>已实现盈亏</dt><dd>{money.format(position.realizedPnl)}</dd></div><div><dt>费用 / 税费</dt><dd>{money.format(position.fees + position.taxes)}</dd></div><div><dt>账本证据</dt><dd>{position.transactionEvidenceIds.length}</dd></div></dl></article>)}</div></div>; }

function Activity({ data, onEvidence }: { data: RiskDashboardData; onEvidence: (id: string) => void }) { return <div className="risk-view"><header className="view-intro"><p>追加式交易账本</p><h2>{data.transactions.length} 条账本事件。</h2><span>更正通过新的 POSITION_ADJUSTMENT 事件完成，历史不会被覆盖。</span></header><div className="activity-list">{data.activity.map((item) => <button key={item.id} className={`activity-row activity-row--${item.tone}`} onClick={() => onEvidence(item.evidenceId)}><time>{item.time}</time><span className="activity-row__marker"/><div><span>{item.type}</span><h3>{item.title}</h3><p>{item.detail}</p></div><code>{item.evidenceId}</code></button>)}</div></div>; }

function Review({ data, onEvidence, onGenerate, onSaveFeedback, onMemoryStatus, loading, error }: { data: RiskDashboardData; onEvidence: (id: string) => void; onGenerate: () => Promise<void>; onSaveFeedback: (runId: string, input: { helpful: boolean | null; hasFactErrors: boolean; missingKeyFactors: boolean; note: string; itemFeedback: ReviewItemFeedback[] }) => Promise<void>; onMemoryStatus: (id: string, status: "accepted" | "rejected") => Promise<void>; loading: boolean; error: string | null }) {
...
function riskRuleRows(data: RiskDashboardData) {
  return [
    ["最大有效敞口", `${Math.round(data.riskRules.maxEffectiveExposure * 100)}%`],
    ["单标的上限", `${Math.round(data.riskRules.maxSinglePosition * 100)}%`],
    ["主题集中度", `${Math.round(data.riskRules.maxThemeConcentration * 100)}%`],
    ["盘中行情新鲜度", `${data.riskRules.quoteStaleSeconds} 秒`],
    ["交易计划", `${data.tradePlans.length} 条`],
  ];
}

function RiskRulesEditor({ data, onSave }: { data: RiskDashboardData; onSave: (rules: RiskRules) => Promise<void> }) {
  const [draft, setDraft] = useState(() => ({
    maxEffectiveExposure: String(Math.round(data.riskRules.maxEffectiveExposure * 100)),
    maxSinglePosition: String(Math.round(data.riskRules.maxSinglePosition * 100)),
    maxThemeConcentration: String(Math.round(data.riskRules.maxThemeConcentration * 100)),
    quoteStaleSeconds: String(data.riskRules.quoteStaleSeconds),
  }));
  const update = (key: keyof typeof draft, value: string) => setDraft((current) => ({ ...current, [key]: value }));
  const parsed: RiskRules = {
    maxEffectiveExposure: Number(draft.maxEffectiveExposure) / 100,
    maxSinglePosition: Number(draft.maxSinglePosition) / 100,
    maxThemeConcentration: Number(draft.maxThemeConcentration) / 100,
    quoteStaleSeconds: Number(draft.quoteStaleSeconds),
  };
  const valid = Object.values(parsed).every((item) => Number.isFinite(item) && item > 0);
  return <section><h3>风险规则</h3>{riskRuleRows(data).map(([label, setting]) => <div className="setting-row" key={label}><span>{label}</span><strong>{setting}</strong></div>)}<div className="risk-rule-editor"><label><span>最大有效敞口 %</span><input inputMode="decimal" value={draft.maxEffectiveExposure} onChange={(event) => update("maxEffectiveExposure", event.target.value)}/></label><label><span>单标的上限 %</span><input inputMode="decimal" value={draft.maxSinglePosition} onChange={(event) => update("maxSinglePosition", event.target.value)}/></label><label><span>主题集中度 %</span><input inputMode="decimal" value={draft.maxThemeConcentration} onChange={(event) => update("maxThemeConcentration", event.target.value)}/></label><label><span>行情过期秒数</span><input inputMode="numeric" value={draft.quoteStaleSeconds} onChange={(event) => update("quoteStaleSeconds", event.target.value)}/></label><button className="primary-action" disabled={!valid} onClick={() => void onSave(parsed)}>保存规则并重算</button></div></section>;
}

function TradePlansEditor({ data, onSave }: { data: RiskDashboardData; onSave: (plans: TradePlan[]) => Promise<void> }) {
  const [draft, setDraft] = useState(() => Object.fromEntries(data.tradePlans.map((plan) => [plan.instrumentId, String(Math.round(plan.maxWeight * 100))])));
  const update = (instrumentId: string, value: string) => setDraft((current) => ({ ...current, [instrumentId]: value }));
  const parsed = data.tradePlans.map((plan) => ({ ...plan, maxWeight: Number(draft[plan.instrumentId]) / 100 }));
  const valid = parsed.every((plan) => Number.isFinite(plan.maxWeight) && plan.maxWeight > 0);
  const nameOf = (instrumentId: string) => data.instrumentMetadata.find((item) => item.id === instrumentId)?.name ?? instrumentId;
  return <section><h3>交易计划</h3>{data.tradePlans.map((plan) => <label className="trade-plan-row" key={plan.instrumentId}><span>{nameOf(plan.instrumentId)}</span><input inputMode="decimal" value={draft[plan.instrumentId] ?? ""} onChange={(event) => update(plan.instrumentId, event.target.value)}/><strong>%</strong></label>)}{!data.tradePlans.length && <p className="empty-note">当前没有交易计划；无计划标的会在复盘中显示为缺失。</p>}<button className="primary-action" disabled={!valid || !data.tradePlans.length} onClick={() => void onSave(parsed)}>保存计划并重算</button></section>;
}
...
interface RiskWorkbenchProps {
  ...
  onActionChange?: (action: RiskAction | undefined) => void;
}
...
export default function RiskWorkbench({
  ...
  const workspace = useRiskWorkspace();
  ...
  const preview = useMemo(
    () => csvText ? workspace.previewCsv(csvText, mapping) : null,
    [csvText, mapping, workspace.data],
  );
  const navigate = (next: View) => {
    setView(next);
    onViewChange?.(next);
  };
  const changeAction = (next: RiskAction | undefined) => {
    setImportOpen(next === "import");
    setHoldingsOpen(next === "holdings");
    onActionChange?.(next);
  };

  useEffect(() => { setView(initialView); }, [initialView]);
  useEffect(() => {
    setImportOpen(action === "import");
    setHoldingsOpen(action === "holdings");
  }, [action]);

  const hasData = Boolean(workspace.data);
  useEffect(() => {
    if (!hasData || window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    let cleanup = () => {};
    void Promise.all([import("gsap"), import("gsap/ScrollTrigger")]).then(([module, trigger]) => {
      const gsap = module.default;
      gsap.registerPlugin(trigger.ScrollTrigger);
      const context = gsap.context(() => gsap.from(".risk-hero > div:first-child > *", {
        y: 32,
        opacity: 0,
        duration: 0.8,
        stagger: 0.08,
  ...
  const loadingClassName = embedded
    ? "risk-loading risk-loading--embedded"
    : "risk-loading";
  if (workspace.loading && !workspace.data) {
    return <div className={loadingClassName}>正在从本地账本重建风险工作区…</div>;
  }
  if (!workspace.data) {
    return <div className={loadingClassName}>{workspace.error ?? "工作区暂不可用"}</div>;
  }

  const data = workspace.data;
  const evidence = data.evidence.find((item) => item.id === selectedEvidence);
  const openEvidence = (id: string) => {
    if (data.evidence.some((item) => item.id === id)) setSelectedEvidence(id);
  };
  const runWorkflow = (step: DailyWorkflowStep) => {
    if (step.id === "transactions") changeAction("import");
    else if (step.id === "reconciliation") navigate("positions");
    else if (step.id === "market" || step.id === "risk") void workspace.reload();
    else if (step.id === "review") navigate("review");
    else if (step.id === "complete") {
      if (data.reviewRuns.some((run) => run.reviewDate === data.analysisDate)) {
        void workspace.completeToday();
      } else navigate("review");
    }
  };

  return (
    <div className={embedded ? "risk-workbench risk-workbench--embedded" : "risk-app risk-workbench"}>
      {!embedded && (
        <header className="risk-appbar">
          <a href="/lab" className="risk-brand">
            <span className="risk-brand__mark">Z</span>
            <span><strong>持仓风险台</strong><small>Evidence before narrative</small></span>
          </a>
          <nav aria-label="风险工作台导航">
            {navItems.map((item) => (
          ...
          <div className="risk-appbar__actions">
            <a className="risk-appbar__link" href="/lab/market">行情中心</a>
            <button onClick={() => changeAction("import")}>导入 CSV</button>
            <button onClick={() => changeAction("holdings")}>券商持仓</button>
            <div className="risk-appbar__status">
              <span className={data.dataMode === "api" && !marketUnavailable(data) ? "is-live" : "is-mock"}/>
              <div><strong>{marketModeLabel(data)}</strong><small>{marketStatusDetail(data)}</small></div>
            </div>
          </div>
        </header>
      )}
      <main className="risk-main">
        <header className="risk-hero">
          <div>
            <p>个人持仓风险监控与操作复盘</p>
            <h1>先把风险算清楚，<br/><span>再解释今天发生了什么。</span></h1>
          </div>
          <div className="risk-hero__aside">
            <p>只读系统</p>
            <strong>{data.riskEvents.length}</strong>
            <span>个活跃风险事件</span>
            <small>{data.portfolio.reliable ? "数据质量通过" : "可信度已降低"}</small>
          </div>
        </header>
        {view === "dashboard" && <Dashboard data={data} onEvidence={openEvidence} onNavigate={navigate} onWorkflow={runWorkflow}/>}
        {view === "positions" && <Positions data={data} onSaveBroker={workspace.saveBrokerQuantity} onOpenHoldingsPaste={() => changeAction("holdings")}/>}
        {view === "activity" && <Activity data={data} onEvidence={openEvidence}/>}
        {view === "review" && <Review data={data} onEvidence={openEvidence} onGenerate={workspace.generateReview} onSaveFeedback={workspace.saveFeedback} onMemoryStatus={workspace.setMemoryStatus} loading={workspace.reviewLoading} error={workspace.reviewError}/>}
        {view === "settings" && <Settings data={data} onRiskRules={workspace.saveRiskRules} onTradePlans={workspace.saveTradePlans} onImport={() => changeAction("import")} onClear={workspace.clear} onExportBackup={workspace.exportBackup} onPreviewBackup={workspace.previewBackup} onRestoreBackup={workspace.restoreBackup}/>}
      </main>
      {!embedded && (
        <footer className="risk-footer">
          <span>zxlab / risk</span>
          <p>确定性计算来自纯函数 Risk Engine；自然语言仅用于解释。</p>
          <a href="/lab">返回 Lab</a>
        </footer>
      )}
