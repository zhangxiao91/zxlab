...
interface PublicStatusResponse {
  ...
  stale: boolean;
  devices: Array<{ name: string; presence: "online" | "recently_online" | "offline"; batteryPercent?: number; batteryLevel?: "high" | "medium" | "low"; charging?: boolean }>;
  activity?: { steps?: number; stepsBucket?: string };
}
...
export const zxtoolkitStatusProvider: StatusProvider = {
  ...
  async getSnapshot(options: StatusProviderOptions = {}) {
    ...
    const devices: DeviceStatus[] = response.devices.map((device, index) => ({
      ...
      lastSeen: updatedAt,
      publicTask: `Battery ${device.batteryPercent != null ? `${device.batteryPercent}%` : device.batteryLevel ?? "not shared"}${device.charging ? " · charging" : ""}`,
      updatedAt
    }));
    ...
    return {
      ...
      activity: unavailable(response.activity?.steps != null ? `Public steps today: ${response.activity.steps.toLocaleString("en")}` : response.activity?.stepsBucket ? `Legacy steps range: ${response.activity.stepsBucket}` : "No public activity field."),
      updatedAt,
